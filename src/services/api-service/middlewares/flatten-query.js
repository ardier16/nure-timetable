/**
 * flattenQuery is needed to provide easier interface for complex query
 ...
 *
 * @param {object} rawQuery - request query with the raw, not formatted properties
 * @return {object} resultQuery - the formatted query object that is valid to use
 * in http-clients
 */
export function flattenQuery (rawQuery) {
  if (!rawQuery) {
    return rawQuery
  }

  if (isDeeperThanOneNesting(rawQuery)) {
    throw new Error('Nested arrays or objects are not allowed for using in query params')
  }

  const resultQuery = {
    ...flattenArraysOnly(rawQuery),
    ...flattenObjectsOnly(rawQuery),
    ...flattenPrimitivesOnly(rawQuery),
  }
